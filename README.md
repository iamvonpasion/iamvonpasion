...
**Backend & Frontend**
`ASP.NET Web API` `React` `Angular` `Next.js` `Sencha` `SignalR` `Electron`

**Mobile**
`React Native` `Ionic`

**Cloud & Infra**
`Azure` `Docker` `Kubernetes` `PostgreSQL` `SQL Server`

**AI & Tooling**
`Claude Code` `Cursor` `OpenAI API` `LiteLLM` `LangChain` `Hugging Face` `Gradio` `MCP Servers`

**Observability**
`Power BI` `Grafana` `SigNoz` `LogRocket` `OpenTelemetry`
